...
import { checkOptProp, checkProp, createTypedArrayGuard, isBoolean, isNull, isNumber, isRecord, isString, propListValue, propValue } from '../_type-utils.ts';
...
export function isParticipantPatch(value: unknown): value is ParticipantPatch {
	return isRecord(value) &&
		...
		checkOptProp(value, 'publishName', isBoolean);
}
...
export function ParticipantPatchFromJSON($value: Record<string, unknown>): ParticipantPatch {
	...
	const publishName = propValue('publishName', $value, isBoolean, 'optional');
	return {
		key,
		version,
		time,
